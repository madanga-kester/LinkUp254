...
public class AuthServices
{
    ...
    public async Task<ProfileCompletenessResult> GetProfileCompletenessAsync(string email)
    {
        try
        {
            var user = await _context.Users.FirstOrDefaultAsync(u => u.Email == email);
            if (user == null)
                return new ProfileCompletenessResult { IsComplete = false, Percentage = 0, Message = "User not found" };

            var fields = new Dictionary<string, bool>
            {
                { "FirstName", !string.IsNullOrEmpty(user.FirstName) },
                { "LastName", !string.IsNullOrEmpty(user.LastName) },
                ...
                { "City", !string.IsNullOrEmpty(user.City) },
                { "Country", !string.IsNullOrEmpty(user.Country) },
                { "ProfilePicture", !string.IsNullOrEmpty(user.ProfilePicture) },
                { "Website", !string.IsNullOrEmpty(user.Website) }
            };

            var weights = new Dictionary<string, int>
            {
                { "FirstName", 10 }, { "LastName", 10 }, { "Email", 10 },
                { "PhoneNumber", 10 }, { "Bio", 15 }, { "DateOfBirth", 10 },
                { "City", 10 }, { "Country", 10 }, { "ProfilePicture", 10 }, { "Website", 5 }
            };

            var totalScore = fields.Sum(f => weights[f.Key] * (f.Value ? 1 : 0));
            var percentage = Math.Min(100, totalScore);
            ...
            return new ProfileCompletenessResult
            {
                ...
                Message = percentage >= 80 ? "Profile complete!" : $"Complete your profile: {string.Join(", ", missingFields.Take(3))}",
                MissingFields = missingFields
            };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "GetProfileCompleteness failed for {Email}", email);
            return new ProfileCompletenessResult { IsComplete = false, Percentage = 0, Message = "Error calculating completeness" };
        }
    }
    ...
    private async Task SendEmailAsync(string toEmail, string otp)
    {
        ...
        {
            Text = $@"<html><body style='font-family:Arial,sans-serif'><h2>🔐 LinkUp254 Verification</h2><p>Your one-time code is:</p><p style='font-size:28px;letter-spacing:8px;font-weight:bold'>{otp}</p><p style='color:#666'>This code expires in 5 minutes.</p></body></html>"
        };
    ...
    {
        _logger.LogInformation("📱 SMS OTP for {Phone}: {Code} (MOCK)", phoneNumber, otpCode);
    }
...
public class AuthResult
{
    ...
    public static AuthResult Success(string message) => new AuthResult { IsSuccess = true, Message = message };
    public static AuthResult Success(string token, User user) => new AuthResult { IsSuccess = true, Token = token, User = user, Message = "Operation successful" };
    public static AuthResult Failure(string message) => new AuthResult { IsSuccess = false, Message = message };
}

// ProfileCompletenessResult class (for profile completion tracking)
